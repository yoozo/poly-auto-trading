...
export const DEFAULT_INDICATOR_PERIOD = 14;
const BOLLINGER_PERIOD = 20;
const BOLLINGER_STDDEV_MULTIPLIER = 2;

export function indicatorWarmupBars(period: number) {
  return Math.max(BOLLINGER_PERIOD, period * 2);
}
...
export function calculateIndicatorPoints(
  ...
  const rsiEmaValues = calculateNullableEmaSeries(rsiValues, period);
