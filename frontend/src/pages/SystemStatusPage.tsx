...
const DEFAULT_PERFORMANCE_RESULTS: PerformanceMetricResult[] = [
  ...
  },
  {
    key: "ws_ping",
    title: "连接后 ping 延迟",
    description: "WS open 后 market.ping 到 market.pong 的 RTT",
    latencyMs: null,
    status: "idle",
    meta: "应用层 ping/pong",
    error: ""
  },
  {
    key: "candles",
    title: "币安 K 线延迟",
    description: "market.candles.request 到 Binance K 线 snapshot",
    latencyMs: null,
    status: "idle",
  ...
  },
  {
    key: "polymarket",
    title: "Polymarket 延迟",
    description: "Polymarket WS 握手到 5m markets.snapshot",
    latencyMs: null,
    status: "idle",
    meta: "BTC Up/Down / 5m",
    error: ""
...
function measurePolymarketMarketSubscribe(
  ...
  return new Promise((resolve) => {
    ...
    const onClose = () => {
      cleanup();
      resolve(metricError(start, "Polymarket WS 已关闭"));
    };
    const onMessage = (event: MessageEvent<string>) => {
      const message = parsePolymarketWsMessage(event.data);
      if (!message || message.request_id !== requestId) return;
