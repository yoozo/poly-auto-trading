...
- `.env` 只保存服务器级主密钥和可选 fallback 配置：
  - `POLYMARKET_CREDENTIALS_ENCRYPTION_KEY`
  - 可选 fallback：`POLYMARKET_CLOB_API_KEY`
  - 可选 fallback：`POLYMARKET_CLOB_SECRET`
  - 可选 fallback：`POLYMARKET_CLOB_PASSPHRASE`
  - 可选 fallback：`POLYMARKET_CLOB_ADDRESS`
  - 可选 fallback：`POLYMARKET_CLOB_SIGNATURE_TYPE`
  - 可选 fallback：`POLYMARKET_CLOB_FUNDER`
...
`POLYMARKET_CLOB_FUNDER` / `funder_address` 应填写 Polymarket Deposit Wallet 地址，通常不是 MetaMask EOA 地址。
...
- 获取方式：
  ...
  - 也可以先使用当前系统已配置的 `position_wallet` 作为候选值，但用户必须确认它确实是 Deposit Wallet。
- `position_wallet` 默认等于 `funder_address`，用于 `/positions?user=<wallet>` 查询仓位。
...
4. 用户确认 profile 信息：
   - `label`
   - `signature_type`，默认 `3`
   - `funder_address`，默认使用当前 `position_wallet` 候选值
   - `position_wallet`，默认等于 `funder_address`
...
- `GET /api/polymarket/credentials`
  - 返回 wallet profiles、active id、masked api key、signer、funder、position wallet、signature type。
...
2. 如果没有 active profile，则 fallback 到 `.env` 单钱包配置。
3. 如果 DB active profile 存在但解密失败，操作失败并给出明确错误，不静默 fallback。
4. 如果没有 active profile 且 `.env` fallback 不完整：
   - positions 可继续使用 `position_wallet` 展示。
   - open orders、撤单、User WS、signed order submit 标记为 credentials missing。

使用 active profile 时：

- account-state 的 `wallet` 使用 `position_wallet`。
...
- 如果没有 active profile，则 fallback 到 `.env` 单钱包配置。
...
- account-state 的 `wallet` 切到 active `position_wallet`。
...
- 提供“生成导入命令”表单：
  ...
  - 切换 Polygon
  - 生成 CLOB credentials
  - 输入/确认 label、funder、position wallet、signature type
  - 生成服务器命令
  - 清空生成结果
